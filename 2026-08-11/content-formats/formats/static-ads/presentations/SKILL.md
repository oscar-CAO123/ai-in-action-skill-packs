...
here is a presentation that was built, rendered, put in front of the operator and either shipped to the
...
|---|---|---|---|---|---|
| **`news-banner/`** | `local:optamize-8-statics/06-editorial-headline-still` | A report about the reader's market, not a message to the reader | plate, full bleed | `f5_variants.build_news` | LIVE on the CRM |
| **`would-you-rather/`** | `local:optamize-8-statics/05-would-you-rather` | A forced choice with a rigged pair, answered before the CTA | black | `f5_variants.build_rather` | LIVE on the CRM |
| **`inbox/`** | `local:optamize-8-statics/04-inbox-screenshot` | Trespass. Mail addressed to somebody else | mail chrome | `f5_variants.build_inbox` | LIVE on the CRM |
| **`apology-letter/`** | `local:optamize-8-statics/03-apology-letter` | The frame swap. An apology promises information, so the body argues while the reader is still reading the apology | warm paper | `f5_variants.build_apology` | LIVE on the CRM |
| **`search-sheet/`** | `local:optamize-6-formats/03-search-bar` | Self-diagnosis. The reader has typed one of those lines themselves | black, white sheet | `f6_variants.build_search` | LIVE on the CRM |
...
Seven rows are on the CRM as of 2026-08-10, all tagged `suite-static`: four `f5-variant` and three
...
3. **Put them side by side in an HTML dossier**, each card beside its own reference and its own
   ...
   one page.
4. **the operator picks by looking.** Do not describe the options in chat, and do not spend a credit
   before he picks.
...
- **Fixed tables, never `Math.random`.** A random renderer means the version the operator approved is not
...
These presentations each break something written down, and each break is the operator's call rather than a
drift. They are listed here so nobody "fixes" them.

| Break | Where | Whose call |
|---|---|---|
| The card comes off black | `apology-letter/`, `ticked-rows/`, `ruled-pad/` | the operator, 2026-08-10 |
| The band law is set aside for a banner | `news-banner/` | the operator, 2026-08-10 |
| Brand furniture on a hand-made card, against `../hand-drawn/SKILL.md` section 2 | `ruled-pad/` (F6 hand) | the operator, 2026-08-10 |
| A negation swap in the copy | `inbox/` | the operator's dictated wording, unruled |
...
- `../../../references/scripts/optamize-formats-teardowns-2026-08-10.md`, the six format verdicts
