#!/usr/bin/env bash
# cio-1981 VO. One your voice model stem per line so line boundaries are exact (the still cuts are
...
KEY=$(grep -m1 '^VOICE_API_KEY=' "$ENGINE/config/.env" | cut -d= -f2- | tr -d '"'"'"' \r')
[ -n "$KEY" ] || { echo "no VOICE_API_KEY"; exit 1; }
mkdir -p vo/stems
...
req=urllib.request.Request('https://api.your voice model.io/v1/text-to-speech/'+sys.argv[2],data=body,
