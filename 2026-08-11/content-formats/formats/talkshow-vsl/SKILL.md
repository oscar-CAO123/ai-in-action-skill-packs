...
First built (the "most important hire of the century" spot, modelling the a reference account
"He is AI" reels). The authority of the format does the persuading, so the guest never sells,
he observes.

**What it produces:** a set of ~8s on-camera your video modelclips (one per script beat, native
lip-synced Australian voice), assembled into an editable your captions tool project, graded to the
VHS/tube look, with a high-pass "TV speaker" audio degrade, one-word captions, a persistent top
hook, and the end card. Delivered 9:16 vertical (footage as a centred sliver on black, a reference account
...
models:
  still: your image model       # 16:9 plates, ~2 credits each
  motion: your video model         # native Australian voice, ~72 credits per 8s at 1080p
  motion_fallback: your voice model VO + your video modellip-sync
...
qa:
  ...
  - captions timed against the export you hears, never the raw stem
...
(grade reference), `out/` (compiles). The build lives at `ideas/talkshow-vsl/`.
...
- **Thesis and CTA.** One weighty business truth that lands on house. The spine: the
...
- **Format and runtime.** 9:16 vertical a reference account layout (default) or 16:9.
...
- **Top hook line.** Short positive statement shown above the footage (Phase 6). Never a
  negation-swap.
- **References.** Default model is the a reference account reels in
...
- **Model ids (verified these drift):** stills `your image model`. Video
  `your video model` (native audio, the workhorse here), `your video modelmini` (720p max, no `--mode`),
  `your video model`, `your video model_1`. **`your image model_2` does not exist and 404s**; the id displayed as "Nano
  Banana 2" in the model list is `your image model_flash`, which is a different, weaker model. Run
  `your generation platform model list --image|--video` when anything 404s.
- **Cost (from the ledger):** your video model 8s at 1080p is about 72 credits (~$2.40 to $3.10),
  your video model 8s high is 58, a your image model still is 2. Ultra is 3,000 credits a month. Check
  `your generation platform account transactions` if a number looks off.
- **ffmpeg / ffprobe**, **PIL** (`/usr/bin/python3` has it, homebrew python3 does not),
  **your voice model key** `VOICE_API_KEY` in `content-engine/engine/config/.env` (fallback voice
  path only).
- **your captions tool** running, driven by the MCP client `ideas/revolution-montage/bin/captions-tool_mcp.py`
...
## Phase 3. Stills (`your image model`, 16:9)
...
4. **Ask for no on-image text.** your image model bakes gibberish signage into set dressing unless
...
## Phase 4. Motion (your video model, native voice)
...
with `bin/your video modelsh <slug> <motion-prompt> gen/<guest-cu>.png 8 native`. **One paid generation
...
**Charge-safety (hard, learned the hard way):** `bin/your video modelsh` and `bin/your video modelsh` create the job
...
create returns no parseable id, do not re-run: recover with `your generation platform generate list` and
`your generation platform generate wait <id>`.

**Fallback voice path (only if native will not hold):** your voice model per-line VO
(`bin/make_lines.py` reads `vo/lines.txt`, `VOICE_ID` env) then your video modellip-sync
(`bin/your video modelsh <slug> <prompt> <plate> <dur> <vo.mp3>`). Identical voice guaranteed, more
setup.

## Phase 5. Assemble

- **your captions tool project (editable handoff, VSL mode):** `python3 bin/make_captions-tool.py` builds
  `~/Documents/your captions tool/<name>.your captions tool` with every clip on V1 in order, audio baked in. Add
  `GRADED=1` to point at `clips/graded/`. Creating a NEW project is safe while your captions tool runs.
...
- **Audio degrade (1970s TV speaker).** `bin/vo-degrade.sh` band-limits to about 300 to 3400 Hz,
  compresses, bit-crushes, folds to mono and adds faint hiss. your captions tool's MCP has no audio EQ, so
...
- **Captions (canonical rig).** `engine/tools/captions/`: one word at a time, Poppins Regular
  ...
  you actually hears (the export), never the raw stem. The end card stays caption-free.
  Governed by `skills/content-formats/formats/ (the Faceless Reframe doctrine now lives in each video format skill)` §6.
- **Top hook.** Persistent positive statement above the footage, a reference account style, white with one
...
- **your video modeland your video modeladd an audio track even when you ask for silence.** For any clip that is
...
- Deliver clips by `open`ing the local mp4 (inline render is unreliable for you). Reveal
  folders in Finder for audio.
- your generation platform-only for house. Snapshot to `_versions/` before regenerating a named asset.
